...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        var bulletPosition = director.position + (director.up * spawnOffset);
        if (!shotgun)
        {
        ...
        else
        {
            ...
            {
                float angle = -shotgunSpread * 0.5f + (i * (shotgunSpread / (shotgunBullets - 1)));
                Quaternion rotation = director.rotation * Quaternion.Euler(0, 0, angle);
                SpawnBullet(bulletPosition, rotation);
            }
        }
